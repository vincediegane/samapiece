...
3. gitleaks en historique complet + PR, non bloquant au demarrage : nouveau workflow secrets-scan.yml, declenche sur push/PR (fetch-depth 0 pour scanner tout l'historique, pas seulement le diff), avec continue-on-error active dans un premier temps pour valider l'absence de faux positifs sur quelques runs avant de le rendre bloquant (a trancher par le spec-writer/codeur selon le resultat des premiers runs). Workflow separe de backend.yml/frontend.yml pour ne pas coupler un faux positif de scan de secrets a la CI applicative existante.
...
- Cout CI du scan historique complet : un scan gitleaks sur tout l'historique a chaque push peut alourdir la CI a mesure que l'historique grandit ; a surveiller, eventuellement passer a un scan diff-only sur PR + scan complet planifie (hebdomadaire) si le temps d'execution devient genant.
