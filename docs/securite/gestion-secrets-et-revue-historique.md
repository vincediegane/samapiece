...
Le scan ponctuel ci-dessus ne protège pas contre une régression future (un développeur qui commettrait accidentellement un secret réel dans un commit ultérieur). Le garde-fou continu est le workflow `.github/workflows/secrets-scan.yml`, en **mode bloquant** (pas de `continue-on-error`), composé de deux jobs à la portée distincte — distinction importante, corrigée après une vérification empirique en CI réelle qui a révélé qu'une première version de ce document affirmait à tort une couverture historique complète continue par un seul job :

- **`gitleaks-diff`**, sur chaque `push`/`pull_request` : exécute `gitleaks/gitleaks-action@v2` avec `.gitleaks.toml`. **Portée réelle : uniquement les commits introduits par l'événement déclencheur** (plage `base..head` calculée par l'action elle-même à partir du contexte push/PR). Contrairement à une affirmation initiale de ce document, `fetch-depth: 0` du `checkout` ne rend cette action capable de scanner que ce que le contexte de l'événement lui indique — il ne fait pas scanner l'historique complet. Vérifié en conditions réelles sur GitHub Actions (run 34835141977) : un push de 4 commits n'a fait scanner que ces 4 commits, pas les 177 commits de l'historique complet du dépôt. Ce job apporte une détection rapide d'un secret nouvellement introduit, pas une garantie de couverture historique.
- **`gitleaks-full-history`**, planifié chaque lundi (`schedule:`) et déclenchable manuellement (`workflow_dispatch`) : exécute l'image Docker officielle `zricethezav/gitleaks:latest` avec `detect --source /repo --log-opts="--all" --config /repo/.gitleaks.toml`. **Portée réelle : tout l'historique du dépôt**, indépendamment du contexte push/PR — c'est ce job, et lui seul, qui rescanne réellement l'ensemble des commits en continu (vérifié : 177 commits scanned lors d'une exécution locale équivalente).

**Ce que cela signifie concrètement pour un lecteur de ce document :** entre deux exécutions planifiées de `gitleaks-full-history` (jusqu'à une semaine), un secret réintroduit profondément dans l'historique par un mécanisme qui ne passe pas par un diff standard de push/PR (ex. rebase, force-push d'un commit ancien) pourrait ne pas être détecté immédiatement par `gitleaks-diff` — il le sera au plus tard lors du prochain rescan hebdomadaire complet, ou peut être déclenché à la demande via `workflow_dispatch`. La seule couverture *immédiate* de l'historique complet à ce jour reste la revue ponctuelle manuelle des sections 4-5 ci-dessus, effectuée une fois ; `gitleaks-full-history` transforme cette revue ponctuelle en un rescan automatisé récurrent, sans pour autant être instantané comme `gitleaks-diff` sur le diff introduit.
